...
def _norm_xy(xy, bbox_min: np.ndarray, span: np.ndarray) -> np.ndarray:
    """Normalize xy into ``[-1, 1]`` relative to local view bbox."""
    u = (np.asarray(xy, dtype=np.float32)[:2] - bbox_min) / span
    return (2.0 * u - 1.0).astype(np.float32)


def _norm_scalar(value: float, scale: float) -> float:
    """Radii / lengths mapped roughly into ``[0, 2]`` so diameter ≈ bbox span → 2."""
    return 2.0 * float(value) / max(float(scale), 1e-6)
...
def tensorize_dxf(dxfir: DxfIR, max_prims: int = MAX_PRIMS) -> dict[str, np.ndarray]:
    ...
    prim_group_ids = np.zeros((max_prims,), dtype=np.int64)
    prim_group_roles = np.zeros((max_prims,), dtype=np.int64)
    ...
    for idx, prim in enumerate(dxfir.prims[:n_prims]):
        ...
        prim_mask[idx] = True
        prim_group_ids[idx] = int(getattr(prim, "group_id", 0) or 0)
        prim_group_roles[idx] = int(getattr(prim, "group_role_id", 0) or 0)
...
def tensorize_dxf_views(
    ...
    *,
    ...
    prim_group_ids = np.zeros((n_views, max_prims), dtype=np.int64)
    prim_group_roles = np.zeros((n_views, max_prims), dtype=np.int64)
    ...
    for v in range(n_views):
        ...
        prim_group_ids[v] = one["prim_group_ids"]
        prim_group_roles[v] = one["prim_group_roles"]
...
def empty_dxf_tensors(max_prims: int = MAX_PRIMS) -> dict[str, torch.Tensor]:
    return {
        ...
        "prim_group_ids": torch.zeros((max_prims,), dtype=torch.long),
        "prim_group_roles": torch.zeros((max_prims,), dtype=torch.long),
...
def empty_dxf_view_tensors(
    *,
    ...
    return {
        ...
        "prim_group_ids": torch.zeros((n_views, max_prims), dtype=torch.long),
        "prim_group_roles": torch.zeros((n_views, max_prims), dtype=torch.long),
...
def tensors_to_torch(tensors: dict[str, np.ndarray]) -> dict[str, torch.Tensor]:
    ...
    for key, value in tensors.items():
        ...
        elif key in {"prim_types", "prim_linetypes", "prim_group_ids", "prim_group_roles"}:
